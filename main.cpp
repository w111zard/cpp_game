...
int main()
{
    ...
    while (true)
    {
        ...
        for (size_t i = 0; i < ENEMIES_COUNT; ++i)
        {
            ...
            level_set(enemies_pos[i], ENEMY_IMG);
        }
        ...
        player_pos.x = x;
        player_pos.y = y;
        level_set(player_pos, PLAYER_IMG);
        player_is_alive = true;

        // ADD COINS
        for (size_t i = 0; i < player_coins_to_win; ++i)
        {
            level_add_randomly(COIN_IMG);
        }

        display_level(level, LEVEL_HEIGHT, LEVEL_WIDTH);
        ...
        while (player_is_alive && (player_collected_coins < player_coins_to_win))
        {
            ...
            char hit_obj = level_get(player_new_pos);
            switch (hit_obj) {
                case SPACE_IMG:
                {
                ...
                case STONE_IMG:
                {
                    ...
                    if (level_get(stone_new_pos) == SPACE_IMG)
                    {
            ...
            for (size_t i = 0; i < ENEMIES_COUNT; ++i)
            {
                ...
                if (level_get(new_enemy_pos) == PLAYER_IMG)
                {
                    player_is_alive = false;
                    break;
                }

                else if (level_get(new_enemy_pos) == SPACE_IMG)
                {
                ...
                else
                {
                    ...
                    for (int j = 0; j < 4; ++j)
                    {
                        ...
                        if (level_get(enemy_pos_in_new_direction) == PLAYER_IMG)
                        {
                            player_is_alive = false;
                            break;
                        }

                        else if (level_get(enemy_pos_in_new_direction) == SPACE_IMG)
                        {
